...
isc.DataSource.addClassMethods({
    ...
    getUpdatedData : function (dsRequest, dsResponse, useDataFromRequest) {
           ...
           ) 
        {
            ...
            //>DEBUG
            this.logDebug("Submitted data to be integrated into the cache:"+ this.echoAll(updateData));
            //<DEBUG
        }
        return updateData;
    },
...
isc.DataSource.addProperties({
    ...
    // <code>&nbsp;&nbsp;&nbsp;&nbsp;var response = { "property":"value1", "property2" : "value2", ... }</code><br>
    // <br>&nbsp;&nbsp;&nbsp;&nbsp;response;</code><br>
    // <P>
...
supportsRequestQueuing : true,
    ...
    //> @attr dataSource.testData       (Array of Object : null : [IR])
...
isc.DataSource.addMethods({
    ...
    getServiceInputs : function (dsRequest) {
        ...
        if (protocol == "getParams" || protocol == "postParams") {
            var params = dsRequest.data,
                defaultParams = operationBinding.defaultParams || this.defaultParams;
            if (defaultParams) params = isc.addProperties({}, defaultParams, params);

            params = this.serializeFields(params, dsRequest);
                                                  ...
                                                  rpcResponse, rpcRequest) 
    {
        ...
        for (var i = 0; i < callbacks.length; i++) {
			...
			if (rpcRequest.willHandleError && callbackResult === false) {
				//>DEBUG
...
//> @type DSProtocol
// +link{dataSource.dataProtocol} affects how the data in the DSRequest (+link{dsRequest.data})
// is sent to the +link{dataSource.dataURL}.  Listed below are the valid values for
// +link{dataSource.dataProtocol} and their behavior.
// <P>
	...
	recordsMatchingFilter : function (records, filter, requestProperties) {
        ...
        for (var i = 0, l = records.length; i < l; i++) {
            ...
            for (j = 0; j < filterFieldsLength; j++) {
                ...
                // Ignore any fields that aren't present in the DataSource.  This allows, eg,
                // totally unrelated criteria
                if (fieldName == null || !this.getField(fieldName)) continue;
