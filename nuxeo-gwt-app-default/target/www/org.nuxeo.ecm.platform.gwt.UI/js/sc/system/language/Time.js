...
isc.Time.addClassProperties({
    ...
    // <P>
    // In order to have times display consistently across browsers all times are stored as
    // UTC time by default.
    // <P>
    // This property may be modified to change the timezone used to store times - for example
    // if you have legacy server data with date-time data in PST and you need it to be 
    // consistently stored and updated in this format, modify this property to be the number
...
isc.Time.addClassMethods({
    ...
    createDate : function (hours, minutes, seconds, milliseconds) {
        ...
        // Catch the case where the UTC hours offset pushes the time past midnight or below zero
        // Note this logic means we differ from the behavior of parseInput - we would roll 
        // times greater than 24 around the clock.
        while (hours >= 24) hours -= 24;      
        while (hours < 0) hours += 24;
        date.setUTCHours(hours);
