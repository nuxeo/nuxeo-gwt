        ...
        if (valueMap != null) {
            if (isc.isAn.Array(valueMap)) {
                if (valueMap.length != 2 || 
                    !((valueMap[0] && !valueMap[1]) || (!valueMap[0] && valueMap[1])) ) valueMap = null;
                ...
                for (var key in valueMap) {
                    ...
                    // Special case - "true" and "false" as a key should be treated as a boolean
                    if (key == this._$false) key = false;
                    else if (key == this._$true) key = true;
                    keys[keys.length] = key;
                    vals[vals.length] = valueMap[key];
                }
