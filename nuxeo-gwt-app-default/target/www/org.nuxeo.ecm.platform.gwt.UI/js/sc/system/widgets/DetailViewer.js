...
isc.DetailViewer.addProperties({
    ...
    // Return the CSS class for a cell. Not implemented by default.
    // The state of the record is indicated by adding a suffix to the base style.
    // There are three independant boolean states, which are combined in the order given:
    // <ol>
    ...
    // <li>"Down" : mouse button is down over this cell
    // </ol>
    // For example, with a baseStyle of "myCell", a cell which is selected, which the mouse is over,
    // will get a styleName of myCellSelectedOver.
    // <p>
    // Cell Styles customizable by:
    // <ul>
    // <li>implementing this function 
    // <li>setting +link{detailViewer.cellStyle}
    // <li>overriding +link{detailViewer.getCellStyle()}
    // </ul>
    //
...
setData : function (newData) {
    ...
    if (this.data && this.data.dataChanged) {
        this.observe(this.data, "dataChanged", "observer.markForRedraw()");
    }
...
//			return the HTML for either a single object or a set of objects expressed as columns
//		@return	(string)	HTML to display
//<
getBlockHTML : function (valueList) {
	...
	var output = "<TABLE BORDER=0 CELLSPACING=0 CLASS=" + this.blockStyle 
            + " WIDTH=" + (this.useInnerWidth ? this.getInnerWidth() : "'100%'")
...
output_binary : function (fieldNum, field, valueList) {
	// output the label
	var output = "<TD WIDTH=10% CLASS='" + this.labelStyle + "' ALIGN=RIGHT"
	...
	for (var i = 0; i < valueList.getLength(); i++) {
		...
		output += "<TD CLASS='" + this.cellStyle + "'>" + value + "<\/TD>";
	}
	return output;
...
output_value : function (fieldNum, field, valueList) {
    ...
    // Note: valueList is really a record object, or an aray of record objects.

	// output the label
    var output;
    if (this.showLabel) {
        output = "<TD WIDTH=10% CLASS='" + this.labelStyle + "' ALIGN=RIGHT"
            + (this.wrapLabel ? ">" : " NOWRAP><NOBR>")
            + this.labelPrefix + (field.title ? field.title : field[this.fieldIdProperty]) + this.labelSuffix 
            + "<\/NOBR><\/TD>"
        ;
	...
	for (var i = 0; i < valueList.getLength(); i++) {
        ...
        if (field.type == "image") {
            ...
            var src = this.getCellValue(valueList.get(i), field),
                prefix = field.imageURLPrefix || field.baseURL || field.imgDir
            ;
            value = this.imgHTML(src, width, height, null, field.extraStuff, 
                                  prefix, field.activeAreaHTML);    
        } else {
            value = this.getCellValue(valueList.get(i), field);
        }
		// output the value as a cell
        var cellStyle;
        if (field.getCellStyle) cellStyle = field.getCellStyle(value, field, record, this);
        else cellStyle = (this.getCellStyle(value, field, record, this) || this.cellStyle);
...
// - create a style name based on the result of +link{detailViewer.cellStyle()} and the 
...
// Cell Styles customizable by:
// <ul>
// <li>implementing +link{detailViewerField.getCellStyle()} 
// <li>setting +link{detailViewer.cellStyle}
...
getCellValue : function (record, field) {
    ...
    if (isc.isA.String(field.formatCellValue)) {
        field.formatCellValue = isc.Func.expressionToFunction("value, record, field",
                                                                field.formatCellValue);
    }
    if (field.getCellValue != null) {
        if (isc.isA.String(field.getCellValue)) {
            field.getCellValue = isc.Func.expressionToFunction("value, record, field",
                                                                field.getCellValue);
        }
        ...
        value = field.getCellValue(value, record, field);
        if (field.formatCellValue) value = field.formatCellValue(value, record);
    } else {
...
output_header : function (fieldNum, field, valueList) {
	return "<TD COLSPAN=" + (valueList.getLength() + 1) + " CLASS='" + this.headerStyle + "'>"+field.value+"</TD>";
},
...
isc.DetailViewer.registerStringMethods({
    ...
    formatCellValue:"value,record,field"
});
