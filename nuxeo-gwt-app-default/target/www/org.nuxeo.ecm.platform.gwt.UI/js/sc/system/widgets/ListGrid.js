...
isc.defineClass("GridBody", isc.GridRenderer).addProperties({
        ...
        if (fitHorizontal || fitVertical) {
            ...
            if (fitVertical) {
                ...
                if (rows > 0) {
                    ...
                    if (this.autoFitMaxHeight && rowHeights > this.autoFitMaxHeight) {
                        rowHeights = this.autoFitMaxHeight;                 
                        vscrollOn = true;
                    }
            ...
            if (height != null && height != this._specifiedHeight) {
                ...
                if (hscrollOn) {
                    ...
                    if (this.autoFitMaxHeight && height > this.autoFitMaxHeight) {
                        height = this.autoFitMaxHeight;
                    }
                }
            }
        ...
        if (!lg.editByCell) {
            ...
            for (var i = 0; i < editItems.length; i++) {
                ...
                if (index == -1) {
                    ...
                    // about to redraw
                    editItems[i].width = completeWidths[index];
            ...
            if (!lengthsMatch || changed) {      
                ...
                for (var i = 0; i < fields.length; i++) {                    
                    if (!itemNames.contains(fieldNames[i])) {
                        var colNum = this.fields.indexOf(fields[i]);
    ...
    // override _rowSelectionChanged to update selectionCanvas if necessary
    _rowSelectionChanged : function (a,b,c,d) {
        var lg = this.parentElement;
        if (lg && lg.showSelectionCanvas) lg.updateSelectionCanvas();
        return this.invokeSuper(isc.GridBody, "_rowSelectionChanged", a,b,c,d);
    },
...
isc.ListGrid.addProperties( {
    ...
    // +link{listGridField.editorValueIcons}.<br>
	...
	//> @attr listGridField.showIf (string : null : [IRWA])
	// A string of script that, if provided, is evaluated to conditionally determine whether
    ...
    _formatLinkCellValue : function (value, field, grid, record, rowNum, colNum) {
        ...
        // link text to show
        var linkText = (record && record.linkText) ? record.linkText : field.linkText || value;
	...
	//>	@attr	listGrid.selectionType		(SelectionStyle : isc.Selection.MULTIPLE : [IRW])
	...
	//> @attr	listGrid.rowEndEditAction (RowEndEditAction : "next" : [IRWA])
	...
	//> @attr	listGrid.listEndEditAction (RowEndEditAction : "stop" : [IRWA])
	...
	//> @attr	listGrid.enterKeyEditAction (EnterKeyEditAction : "done" : [IRWA])
	// What to do when a user hits enter while editing a cell:
    // <ul>
    ...
    //> @attr	listGrid.escapeKeyEditAction (EscapeKeyEditAction : "cancel" : [IRWA])
    ...
    //> @attr listGrid.sortField (field name or integer : null : IR)
...
setAutoFitMaxHeight : function (height) {
    ...
    if (this.body) {
        this.body.autoFitMaxHeight = height;
        this.body.adjustOverflow();
    }
},
...
_startFolderAnimation : function (folder) {
    ...
    this.startRowAnimation( true, 
                            folderIndex+1, 
                            (folderIndex + numChildren+1), 
                            {target:this.body, methodName:"redraw"}, 
                            this.animateFolderSpeed, 
                            this.animateFolderTime,
                            this.animateFolderEffect,
                            true
...
dataChanged : function (type, originalRecord, rowNum, updateData) {
    ...
    if (groupByFields != null && !this._markForRegroup) {
        ...
        if (type == "add" || type == "remove") this._markForRegroup = true;
        if (type == "update" && rowNum != null) { 
            var currData;
            if (this.data.isGroupedOutput && this.originalData) currData = this.originalData;
            else currData = this.data;
...
setFields : function (newFields) {
    ...
    if (!this.booleanFalseImage && !this.booleanTrueImage) { 
        ...
        this.booeanFalseImage = isc.CheckboxItem ? 
    ...
    if (this.showHeader && this.headerHeight > 0 && this.header != null) { 
        ...
        if (this.body != null) this.syncHeaderScrolling();
    }
...
getCellCSSText : function (record, rowNum, colNum) {
    ...
    // support hilites
    if (!this.hilites || record[this.hiliteProperty] == null) return cssText;

    cssText = this._addHiliteCSSText(record, colNum, cssText);
    return cssText;
...
getCellValue : function (record, recordNum, fieldNum, gridBody) {
    ...
    if (!isEditCell) {
        ...
        if (icon != null) {
            ...
            iconHTML = isc.Canvas._getValueIconHTML(icon, prefix, width, height,
                                              leftPad, rightPad,
                                              // no need for an ID
                                              null,
...
// Expected values are: 'right', 'center', or 'left'.
//
// @see getCellStyle()
//
...
// Hilites
// ---------------------------------------------------------------------------------------
// Per-cell styling that can be declared on records
...
setHilites : function (hilites) {
    ...
    this._hiliteIndex = this.hilites.makeIndex("id");
...
applyHilites : function () {
    ...
    data.setProperty(this.hiliteProperty, null);
...
getHilite : function (hiliteId) {
    ...
    return (this._hiliteIndex ? this._hiliteIndex[hiliteId] : null);
},
...
applyHilite : function (hilite, data, fieldName) {
    ...
    // hilite may be applied in some other way
    ...
    this.logWarn("applying filter: " + isc.Comm.serialize(hilite.criteria) + 
...
hiliteRecord : function (record, field, hilite) {
    var hiliteCount = record[this.hiliteProperty];
    if (hiliteCount == null) hiliteCount = record[this.hiliteProperty] = this._hiliteCount++;
...
_hiliteIterator : [],
_addHiliteCSSText : function (record, colNum, cssText) {
    var hiliteCount = record[this.hiliteProperty],
    ...
    for (var i = 0; i < hiliteIds.length; i++) {
        var hilite = this.getHilite(hiliteIds[i])
        if (hilite != null) {
            cssText = cssText ? cssText + isc.semi + hilite.cssText : hilite.cssText;
        }
    }
...
editHilites : function () {
    ...
    var theWindow = this.hiliteWindow = isc.Window.create({
        autoDraw:true,
        items : [
            isc.HiliteEditor.create({
...
syncHeaderScrolling : function (left, top) {
	// make the header scroll horizonally if necessary
...
getFilterEditorType : function (field) {
    ...
    return field.filterEditorType || field.editorType || field.formItemType;
},
...
_storeEditValues : function (rowNum, colNum, editValues, editValuesId) {
    ...
    if (record == null) {
        ...
        for (var i = 0; i < fields.length; i++) {
            var field = fields[i],
                fieldName = field[this.fieldIdProperty];
            if (field.defaultValue != null && editValues[fieldName] === undef) {            
...
setEditValue : function (rowNum, colNum, newValue, suppressDisplay, suppressChange) {
    ...
    if (field && this._useDisplayFieldValue(field)) {
        ...
        if (displayValue == null) {
            ...
            var data = this.data;
            if (isc.isA.ResultSet(data)) data = data.localData;
            if (data) {
...
setFieldError : function (rowNum, fieldID, errorMessage, dontDisplay) {
    ...
    if (errorMessage == null || (isc.isAn.Array(errorMessage) && errorMessage.length == 0)) {
        ...
        if (data == null) data = this.createEditSession(rowNum, this.getColNum(fieldID));
...
setRowErrors : function (rowNum, errors, dontDisplay) {
    ...
    if (data == null) data = this.createEditSession(rowNum);
...
removeRecord : function (rowNum, record) {
    ...
    if (animateRemoveRecord) {
        ...
        this.body._initializeShowHideRow(false, rowNum, rowNum+1);
    }
    ...
    if (ds) {
        ...
        if (isc.isAn.Array(this.data)) {
            if (!this.data.contains(record)) return;
...
removeDataComplete : function (rowNum, record, response) {
    ...
    if (!record) {
        ...
        this.startRowAnimation(false, rowNum, rowNum+1, {target:this, methodName:"redraw"},
...
//<
startRowAnimation : function (show, startRow, endRow, callback, speed, duration, effect, slideIn, delayed) {
    if (!this.body) return;
    this.body.startRowAnimation(show, startRow, endRow, callback, speed, duration, effect, slideIn, true, delayed);
},
...
//     this list - this is an attempted drag-reorder. If +link{ListGrid.canReorderRecords} is 
//     false, this method returns false.</li>
// <li>If the dragTarget is some other widget, and +link{ListGrid.canAcceptDroppedRecords} is 
...
// the field to be used in the show / hide fields context menu.
//		@group	display
//
...
toggleFrozen : function (field, isFrozen) {
    ...
    if (!field || !this.fieldIsVisible(field)) return;
...
_addHeaderSpans : function (fields, header) {
	...
	for (var i = 0; i < headerSpans.length; i++) {
		...
		span._spanAutoSizeHeight = true;
		if (this.headerSpanHeight == null) {
...
// @visibility external
//<
// @param [suppressFieldWidthsReorder] (boolean) Internal parameter to suppress actually reordering
// this._fieldWidths. This is used for filterEditors where the fieldWidths array is shared between
// the source widget and the editor widget, so has already been reordered when this method is called
reorderFields : function (start, end, moveDelta, fieldWidthsUpdated) {
    ...
    if (!fieldWidthsUpdated) this._fieldWidths.slideRange(start, end, start+moveDelta);
    ...
    if (this.showFilterEditor) {        
        this.filterEditor.reorderFields(start, end, moveDelta, true);
    }
...
getToggleFreezeText : function (field) {
    return (field.frozen ? "Unfreeze " : "Freeze ") + this.getSummaryTitle(field);
},
...
getGroupByText : function (field) {
    return "Group by " + this.getSummaryTitle(field);
},
...
shouldShowColumnPicker : function (fld) {
    // don't allow hiding treeField columns - it doesn't make much sense and is not
    // currently supported
    if (fld.treeField) return false;
...
regroup : function (fromSetData) { 
	...
	if (!this.isGrouped || (this.canGroupBy == false) || !this.groupByField || 
        !this.groupByField.length || this.inhibitRegroup || !this.data || 
        !this.data.getLength()) 
    {
        return;
	}
...
isc.defineClass("HiliteEditor", "SectionStack").addProperties({
    ...
    hiliteListDefaults : {
        ...
        fields : [
            ...
            {name:"name", title:"Sample Highlight", formatCellValue:"'Sample Text'",
    ...
    initWidget : function () {
        ...
        this.hilitesDS = this.createAutoChild("hilitesDS", {
            testData : this.grid ? this.grid.getHilites() : []
        });
        ...
        this.addSection({ 
            ...
            items : ["autoChild:hiliteList", "autoChild:importButton" ]
        });
        this.addSection({
        ...
        this.addAutoChild("filterBuilder", {dataSource:this.dataSource});
        this.addAutoChild("hiliteForm", {dataSource:this.hilitesDS});
        this.hiliteForm.setValueMap("fieldName", this.getDataSource().getFieldNames());

        this.addAutoChildren(["hiliteButtons", "addButton", "newButton"]);
    ...
    saveHilite : function () {
        ...
        var editor = this;
        this.hiliteForm.saveData(function () {
            if (editor.grid) {
                isc.logWarn("setting hilites to: " + isc.echoAll(editor.hilitesDS.testData));
                editor.grid.setHilites(editor.hilitesDS.testData);
            }
        });
    }
});
