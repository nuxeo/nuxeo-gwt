...
// Layout subclass that automatically applies resizers to all members except
// the last one.
// ---------------------------------------------------------------------------------------
isc.defineClass("AllMemberResizeLayout", "Layout").addProperties({
    ...
    reorderMembers : function () {
        this.Super("reorderMembers", arguments);
        this.membersChanged();
    },
    
    membersChanged : function () {
        var showResizer = false;
        for (var i = this.members.length-1; i >= 0; i--) {
            var member = this.members[i];
            if (showResizer) {
                if (!member.noResizer) member.setShowResizeBar(true);
            } else {
                if (member.isVisible()) showResizer = true;
                member.setShowResizeBar(false);
            }
        }
    }
});
...
isc.defineClass("PortletContainer", "AllMemberResizeLayout", "EditContext").addProperties({

getDroppedPortlet : function () {
    var source = isc.EH.dragTarget;

    // portlet moved
    if (!isc.isA.Palette(source)) return source;
...
isc.defineClass("PortalRow", "PortletContainer").addProperties({
    ...
    drop : function () {
        if (this.isHDrop()) {
            var portlet = this.getDroppedPortlet();
            if (!this.hasMember(portlet)) this.addMember(portlet,  this.getDropPosition());
            else this.Super("drop", arguments); 
    ...
    membersChanged : function () {
        ...
        else this.Super("membersChanged", arguments);
    }
...
isc.defineClass("PortalLayout", "AllMemberResizeLayout").addProperties({
    vertical:false,
    numColumns:2,
