...
_calculateBorderSize : function () {
    ...
    if (!this.isDrawn()) {
        ...
        if (borderString != null && !isc.isA.String(borderString)) {
            //>DEBUG
            this.logWarn("this.border defined as " + borderString + 
                         ". This property should have a string value - converting to a string.");
            //<DEBUG
            this.border = borderString + "";
        }
...
updateHover : function (hoverHTML) {
    if (isc.Hover.lastHoverCanvas != this) return;
    if (hoverHTML == null) hoverHTML = this.getHoverHTML();
    if (hoverHTML == null || isc.isAn.emptyString(hoverHTML)) isc.Hover.hide();
    
    isc.Hover.hoverCanvas.setContents(hoverHTML);
...
// generation. 
getPrintHTML : function (components, printProperties, callback, HTML, index) {
    ...
    var async,
        componentCallback = {target:this, methodName:"gotComponentPrintHTML",
                             ...
                             callback:callback, HTML:HTML, index:index};
    ...
    if (callback) {
        this.fireCallback(callback, "HTML,callback", [HTML.join(isc.emptyString), callback]);
    }
