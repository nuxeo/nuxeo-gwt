...
getOffsetLeft : function (element) {
    ...
    if (isc.Browser.isSafari) {
        // In Safari, if the offsetParent has a border, the offsetLeft / top reported is 
        // relative to the outside of that border, rather than the inside, so deduct that value
        ...
        if (isc.Browser.isIE || isc.Browser.isOpera) {
            ...
            if (isc.Browser.isOpera && 
                (isLeft ? styleObj.borderLeftStyle == this._$none 
                        : styleObj.borderTopStyle == this._$none)) borderWidth = 0;
