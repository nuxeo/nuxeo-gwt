...
// @visibility internal
//<
// NOTE: intentionally zero doc of the functionality for now, in anticipate of a full color
// wheel interface with radically different configuration.
...
isc.ColorItem.addProperties({
    ...
    pickerDefaults: {
        ...
        // notification method to fire pickerDataChanged instead.
        colorSelected : function (color) { this.callingFormItem.pickerColorSelected(color) }
    },
    
    
    //>	@attr	colorItem.showPickerIcon    (boolean : true : IRW)
    //      Should we show the pick button icon for choosing colors?
    ...
    // The ColorPicker associated with this ColorItem can operate in either a "simple" mode 
    ...
    // Determines whether this ColorItem contains transparency/opacity information. If set,
    // color transparency are 100% opaque.
    // Determines the title / icon shown on the picker for select null values   
    //<
...
isc.ColorItem.addMethods({
    ...
    // This is a FormItem, it doesn't extend Canvas so there's no initWidget...
    init : function() {
	...
	//<
	pickerColorSelected : function (color) {
