...
isc.PickListMenu.addMethods({
    ...
    _formatCellValue : function (value, record, rowNum, colNum) {
        if (this.formItem.formatPickListValue) {
            var fieldName = this.getFieldName(colNum);
            value = this.formItem.formatPickListValue(value, fieldName, record);
        }
        return this.Super("_formatCellValue", [value,record,rowNum,colNum]);
    },
...
isc.PickList.addInterfaceMethods({
   ...
   setUpPickList : function (show, queueFetches, request) {
    ...
    filterComplete : function (response, data, request) {
        ...
        // Always select the default item at this point
        // (Don't want to select the default item BEFORE the list is shown as the selection
        //  will get wiped out when the data is changed).
        if (list.isVisible()) this.selectDefaultItem();
    ...
    //> @attr PickList.textMatchStyle (boolean : "startsWith" : IR)
